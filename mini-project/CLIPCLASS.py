...
class Clip (MEDIA.Media):
    ...
    def __init__(self,all_info,name=None,director=None,imd=None,url=None,duration=None,cast=None,type='clip'):
    ...
    def search_clip(self):
        ...
        for i in self.clip_list:
            if name == i['name']:
                return Clip(self.clip_list,name).show_info(self.clip_list)

        return print('The product is not available')
    def search_advanced_clip(self,time_min,time_max):
        ...
        for i in self.clip_list:
            time_clip=list(map(int,i['duration'].split(':')))
            secen_movie=int(time_clip[0])*60+int(time_clip[1])
