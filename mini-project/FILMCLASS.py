...
class Film (MEDIA.Media):
    ...
    def __init__(self,all_info,name=None,director=None,imd=None,url=None,duration=None,cast=None,type='film'):
    ...
    def add_film(self):
        ...
        return {'type':self.type,'name': self.name, 'director': self.director, 'IMDB': self.imd_score, 'url': self.url,
             'duration':self.duration,'episodes':self.episides,'casts': self.casts}
        ...
        for i in self.film_list:
            if self.name == i['name']:
                menu_number=int(input('1.name\n'
                                      ...
                                      '5.duration\n'
                                      '6.casts\n'))
                list=['name','director','IMDB','url','duration','casts']
                if menu_number==1:
    ...
    def search_film(self):
        ...
        for i in self.film_list:
            if name == i['name']:
                return Film(self.film_list,name).show_info(self.film_list)

        return print('The product is not available')
    def search_advanced_film(self,time_min,time_max):
        ...
        for i in self.film_list:
            time_film=list(map(int,i['duration'].split(':')))
            secen_movie=int(time_film[0])*60+int(time_film[1])
