...
class Documentary (MEDIA.Media):
    ...
    def __init__(self,all_info,name=None,director=None,imd=None,url=None,duration=None,cast=None,episode=0,type='documentary'):
        MEDIA.Media.__init__(self,name,director,imd,url,duration,cast)
        self.type = type
        self.episodes=episode
        self.documentary_list=[]
        for i in all_info:
            if self.type==i['type']:
                self.documentary_list.append(i)

    def add_documentary(self):
        for i in self.documentary_list:
            if self.name==i['name']:
                return False
    ...
    def edit_documentary(self):
        ...
        return False
    def search_documentary(self):

        name = input('name: ').lower()
        for i in self.documentary_list:
            if name == i['name']:
                return Documentary(self.documentary_list,name).show_info(self.documentary_list)

        return print('The product is not available')
    def search_advanced_documentary(self,time_min,time_max):
        ...
        for i in self.documentary_list:
            time_documentary=list(map(int,i['duration'].split(':')))
            secen_movie=int(time_documentary[0])*60+int(time_documentary[1])
            if secen_movie>secen_min and secen_movie<secen_max:
                print('name: %s'% i['name'])
        time.sleep(3)
    def remove_documentary(self):
        num=0
        for i in self.documentary_list:
            if self.name== i['name']:
                return num
    ...
    def qrrcode_documentary(self):
        for i in self.documentary_list:
            if self.name== i['name']:
                url = pyqrcode.create(i['url'])
                url.png('qrcode-url-documentary.png',scale=6)
                return 'qrcode created'
        return 'There is no movie movie with this name'
    def download_documentary(self):
        for i in self.documentary_list:
            if self.name == i['name']:
                Documentary(self.documentary_list, self.name).download(i['url'])
